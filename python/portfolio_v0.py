...
b = pd.Series(m.addVars(stocks, vtype=gp.GRB.INTEGER), index=stocks)
...
portfolio_risk = sum(np.diag(sigma)[i] * vars[stocks[i]] * b[stocks[i]] for i in range(len(stocks)))
...
min_adq = 0.05
for i in stocks:
    m.addConstr(vars[i] >= min_adq * b[i])
...
for v in vars:
    if v.x > 0:
        ...
        print('{} - {} - {}'.format(v.varname, v.x, stocks[v.index]))
...
def plot_portfolio(stock_volatility, minrisk_volatility, minrisk_return, stock_return):
...
plot_portfolio(stock_volatility, minrisk_volatility, minrisk_return, stock_return)
